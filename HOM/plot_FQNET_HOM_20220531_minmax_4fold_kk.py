...
START_TIME_tab1 = '2022-05-31 17:25:00'
END_TIME_tab1 = '2022-05-31 17:35:00'
START_TIME_tab2 = '2022-05-31 17:25:00'
END_TIME_tab2 = '2022-05-31 17:35:00'
...
try:
	...
	with db_gui.cursor(pymysql.cursors.DictCursor) as cur:
		TABLE_NAME = "TAB131_05_2022_12_10_31"
		...
		TABLE_NAME = "TAB231_05_2022_12_10_31"
		queryGUI = "SELECT ch2, ch3, ch4, ch5, datetime FROM "+TABLE_NAME+" WHERE datetime BETWEEN {ts %s} AND {ts %s}"
	...
	two_fold_max = sum(ch0[:half])
	two_fold_min = sum(ch0[half:])
	three_fold_max_a = sum(ch2[:half])
	three_fold_min_a = sum(ch2[half:])
	three_fold_max_b = sum(ch1[:half])
	three_fold_min_b = sum(ch1[half:])
	four_fold_max = sum(ch3[:half])
	four_fold_min = sum(ch3[half:])
	...
	def error(min,max):
		return np.sqrt((np.sqrt(min)/max)**2 + (min*np.sqrt(max)/max**2)**2)

	two_fold_vis_err = error(two_fold_min,two_fold_max)
	three_fold_vis_a_err = error(three_fold_min_a,three_fold_max_a)
	three_fold_vis_b_err = error(three_fold_min_b,three_fold_max_b)
	four_fold_vis_err = error(four_fold_min,four_fold_max)
	...
	print("Two fold visilbility: ",two_fold_vis, " +/- ",two_fold_vis_err)
	...
	print("Three fold heralding Alice visilbility: ",three_fold_vis_a, " +/- ",three_fold_vis_a_err)
	...
	print("Three fold heralding Bob visilbility: ",three_fold_vis_b, " +/- ",three_fold_vis_b_err)
	print("Four fold maximum counts :",four_fold_max)
	print("Four fold minimum counts :",four_fold_min)
	print("four fold visilbility: ",four_fold_vis, " +/- ",four_fold_vis_err)
