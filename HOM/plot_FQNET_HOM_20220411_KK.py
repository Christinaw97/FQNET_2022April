...
START_TIME = '2022-04-11 23:37:00'
END_TIME = '2022-04-12	14:41:00'
...
try:
	...
	with db_gui.cursor(pymysql.cursors.DictCursor) as cur:
		TABLE_NAME = "TAB111_04_2022_23_34_26"
		...
		TABLE_NAME = "TAB211_04_2022_23_34_26"
		queryGUI = "SELECT ch0, ch1, ch2, ch3, ch4, ch5, ch6, ch7, datetime FROM "+TABLE_NAME+" WHERE datetime BETWEEN {ts %s} AND {ts %s}"
		cur.execute(queryGUI, (START_TIME,END_TIME,))
		row = cur.fetchone()
	...
	df_tab2 =pd.DataFrame(list(zip(time_tab2_el, Ch0, Ch1, Ch2, Ch3, Ch4, Ch5, Ch6, Ch7)))
	df_tab2.columns=["time_histo_el_s", "ch0", "ch1", "ch2", "ch3", "ch4", "ch5", "ch6", "ch7"]
	...
	for i in range(len(time_tab2_el)):
		...
		ch7 = ch7 + Ch7[i]
		#print(adqtime/10)
		if i%round(delay_step_times[j]/adqtime_tab2)==0:
			#print(time_tab2_el[i]
			time_tab2_el_bin.append(round(time_tab2_el[i]))
			if delay_step_times[j]<(60*90):
				ch0 = 1.5*ch0
				ch1 = 1.5*ch1
				ch2 = 1.5*ch2
				ch3 = 1.5*ch3
	...
	for i in range(len(time_histo_el)):
		...
		d1 = d1 + D1[i]
		d2 = d2 + D2[i]
		if i%round(delay_step_times[j]/adqtime_histo)==0:
 ...
 #Rb1 = Rb1[histo_indices[0]:histo_indices[-1]]
	...
	tab1_titles = ["Alice", "Bob", "Splitter Alice", "Splitter Bob"]#, ["Rb1", "Rb2", "Rb3"], ["Rc1","Rc2", "Rc3"]]
	...
	for i in range(4):
		...
		if i < 2:
			axs[i].vlines(time_histo_el_bin,100000,300000,color='r')
		else:
	...
	tab2_data = [ch2_bin,ch3_bin,ch4_bin,ch5_bin]
	print(tab2_data)
	tab2_titles = ["2 fold", "3 fold Heralding Alice", "3 fold Heralding Bob", "4 fold"]
	...
	meanCounts=np.mean(a0_bin)
	normA0Array= np.array(a0_bin)/meanCounts
	...
	for i in range(4):
		...
		tab2_data[i][tab2_indices[0]:tab2_indices[-1]]/normA0Array[tab2_indices[0]:tab2_indices[-1]],
		 linestyle = 'none', marker = '.', markersize = 8)
	...
	for i in range(4):
		...
		tab2_data[i][tab2_indices[0]:tab2_indices[-1]]/normB0Array[tab2_indices[0]:tab2_indices[-1]],
		 linestyle = 'none', marker = '.', markersize = 8)
	...
	for i in range(4):
		...
		tab2_data[i][tab2_indices[0]:tab2_indices[-1]]/normB0Array[tab2_indices[0]:tab2_indices[-1]]/normA0Array[tab2_indices[0]:tab2_indices[-1]],
		 linestyle = 'none', marker = '.', markersize = 8)
	...
	hom_counts_no_norm = tab2_data[1][tab2_indices[0]:tab2_indices[-1]]
	hom_counts_norm_both = tab2_data[3][tab2_indices[0]:tab2_indices[-1]]/normA0Array[tab2_indices[0]:tab2_indices[-1]]/normB0Array[tab2_indices[0]:tab2_indices[-1]]

	hom_counts_norm_a = tab2_data[3][tab2_indices[0]:tab2_indices[-1]]/normA0Array[tab2_indices[0]:tab2_indices[-1]]
	hom_counts_norm_b = tab2_data[3][tab2_indices[0]:tab2_indices[-1]]/normB0Array[tab2_indices[0]:tab2_indices[-1]]


	print(hom_counts_no_norm)

	HOM_data = [[sum(steps[:i])*40,h] for i, h in enumerate(hom_counts_norm_b) if i > 1]
	HOM_df = pd.DataFrame(HOM_data,columns=['delay_(ps)','counts_per_90_min'])
	HOM_df.to_csv('HOM_20220411_4f_norm_b.csv',index=False)
