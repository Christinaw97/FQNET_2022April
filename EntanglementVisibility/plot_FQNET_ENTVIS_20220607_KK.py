...
import pandas as pd
delay_step_time = 60
#delay_step_times = delay_step_time*np.ones(9)##how many seconds per step(10 minutes is 600)
adqtime_tab2=2
adqtime_histo=2


START_TIME = '2022-06-07 16:32:28'
END_TIME = '2022-06-07	16:45:38'
...
try:
	...
	with db_gui.cursor(pymysql.cursors.DictCursor) as cur:
		...
		queryGUI = "SELECT ch0, ch1, ch2, ch15, datetime FROM "+TABLE_NAME+" WHERE datetime BETWEEN {ts %s} AND {ts %s}"
		cur.execute(queryGUI, (START_TIME,END_TIME,))
		row = cur.fetchone()
	...
	path_to_file ="/home/fqnet/Software/FQNET/FQNET_2022April/EntanglementVisibility"
	...
	df_tab2 =pd.DataFrame(list(zip(time_tab2_el, Ch0, Ch1, Ch2, Ch15)))
	df_tab2.columns=["time_histo_el_s", "ch0", "ch1", "ch2", "ch15"]
	...
	for i in range(len(time_tab2_el)):
		...
		if j<14 and i%round(delay_step_time/adqtime_tab2)==0:
			time_tab2_el_bin.append(round(time_tab2_el[i]))
			ch0_bin.append(ch0)
			ch1_bin.append(ch1)
			ch2_bin.append(ch2)
			ch15_bin.append(ch15)
			# dlh = time_histo_el[i]
