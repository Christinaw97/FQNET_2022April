...
try:
	...
	IM_pulse_data = np.load('IM_min_max_data.npy')
	IM_min = [float(data[:-1])*1e3 for data in IM_pulse_data[0]]
	IM_max = [float(data[:-1])*1e3 for data in IM_pulse_data[1]]

	bias_mon = np.loadtxt('IM_POM/Bob_IM_input_power.csv',skiprows=15,delimiter=',',dtype='S')
	...
	for i in range(3):
		#for j in range(1):
		axs[i].plot(tab1_data[i][:10800],  linestyle = 'none', marker= '.', markersize=8)
