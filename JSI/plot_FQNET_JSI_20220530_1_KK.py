...
try:
	...
	with db_gui.cursor(pymysql.cursors.DictCursor) as cur:
		...
		TABLE_NAME = "TAB129_05_2022_16_04_42"
		...
		while i < len(time_JSI)-1:
			...
			TABLE_NAME = "TAB229_05_2022_16_04_42"
		...
		ch0_bin.append(sum(Ch0_interm)/len(Ch0_interm))
		ch1_bin.append(sum(Ch1_interm)/len(Ch1_interm))
	...
	df_tab3 =pd.DataFrame(list(zip(ch0_bin, ch1_bin)))
	df_tab3.columns=["ch0", "ch1_bin"]
 ...
 #Rb1 = Rb1[histo_indices[0]:histo_indices[-1]]
	...
	y_b,x_b = np.mgrid[min_wl-0.05:max_wl+delta_wl-0.05:delta_wl, min_wl-0.05-2:max_wl-2-0.05+delta_wl:delta_wl]

	Alice_JSI_raw = np.array(ch0_bin[0:81**2]).reshape(81,81)
	Bob_JSI_raw = np.array(ch1_bin[0:81**2]).reshape(81,81)
	...
	c = ax.pcolormesh(x,y,Alice_JSI_raw)
	...
	c = ax.pcolormesh(x_b,y_b,Bob_JSI_raw)
	ax.set_ylabel('Bob 2')
	ax.set_xlabel('Bob 1')
	ax.set_title("Bob")
	fig1.colorbar(c, ax=ax)
