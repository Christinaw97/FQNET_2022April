...
try:
	...
	with db_gui.cursor(pymysql.cursors.DictCursor) as cur:
		...
		END_TIME = time_JSI[-1]
		END_TIME = END_TIME + datetime.timedelta(seconds=delta_time.total_seconds()+3)
	...
	for i in range(len(time_tab2_el)):
		if i%round(delta_time/adqtime_tab2)>2:
			ch0 = ch0 + Ch0[i]
			ch1 = ch1 + Ch1[i]
		#print(adqtime/10)
	...
	for i in range(len(time_histo_el)):
		if i%round(delta_time/adqtime_histo)>2:
			a0 = a0 + A0[i]
			a1 = a1 + A1[i]
			a2 = a2 + A2[i]
			b0 = b0 + B0[i]
 ...
 #Rb1 = Rb1[histo_indices[0]:histo_indices[-1]]
	...
	Alice_JSI = np.array(ch0_bin[0:441]).reshape(21,21)
	Bob_JSI = np.array(ch1_bin[0:441]).reshape(21,21)
