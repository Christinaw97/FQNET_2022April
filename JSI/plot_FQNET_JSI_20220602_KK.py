...
try:
	...
	with db_gui.cursor(pymysql.cursors.DictCursor) as cur:
		...
		END_TIME = time_JSI[-1]
		END_TIME = END_TIME + datetime.timedelta(seconds=delta_time.total_seconds()+3*25)
		...
		TABLE_NAME = "TAB102_06_2022_17_30_23"
		...
		i = 0
		while i < len(time_JSI)-1:



			TABLE_NAME = "TAB202_06_2022_17_30_23"
			queryGUI = "SELECT ch0, ch1, datetime FROM "+TABLE_NAME+" WHERE datetime BETWEEN {ts %s} AND {ts %s}"
			cur.execute(queryGUI, (time_JSI[i],time_JSI[i+1],))
			row = cur.fetchone()
			Ch0_interm, Ch1_interm = [],[]
			...
			while row is not None:
				...
				Ch0.append(Ch0_val)
				Ch1.append(Ch1_val)
				Ch0_interm.append(Ch0_val)
				Ch1_interm.append(Ch1_val)
				time_tab2.append(row["datetime"])
				row = cur.fetchone()

			ch0_bin.append(sum(Ch0_interm))
			ch1_bin.append(sum(Ch1_interm))
		...
		TABLE_NAME = "TAB202_06_2022_17_30_23"
		queryGUI = "SELECT ch0, ch1, datetime FROM "+TABLE_NAME+" WHERE datetime BETWEEN {ts %s} AND {ts %s}"
		cur.execute(queryGUI, (time_JSI[i],time_JSI[i]+datetime.timedelta(seconds=3),))
		row = cur.fetchone()
		Ch0_interm, Ch1_interm = [],[]
	...
	print("time JSI first=", str(time_JSI[0]))
	print("time JSI last=", str(time_JSI[-1]))
	...
	csv_filename_tab3= cur_file_name+"_tab3.csv"
	path_to_file ="/home/fqnet/Software/FQNET/FQNET_2022April/JSI/"
	...
	df_tab3 =pd.DataFrame(list(zip(time_tab2_el, ch0_bin, ch1_bin)))
	df_tab3.columns=["time_histo_el_s", "ch0", "ch1"]
	df_tab3.to_csv(path_to_file+csv_filename_tab3)
	...
	print("Ch0 length:", len(Ch0))
	print("Ch1 length:", len(Ch1))

	print("I's length:", len(time_tab2_el))

	'''for i in range(len(time_tab2_el)):
		#if i%round(delta_time/adqtime_tab2)>1:
		#	ch0 = ch0 + Ch0[i]
		#	ch1 = ch1 + Ch1[i]
		#print(adqtime/10)
		ch0 = ch0 + Ch0[i]
	...
	for i in range(len(time_histo_el)):
		#if i%round(delta_time/adqtime_histo)>1:
		a0 = a0 + A0[i]
		a1 = a1 + A1[i]
		a2 = a2 + A2[i]
		b0 = b0 + B0[i]
 ...
 #Rb1 = Rb1[histo_indices[0]:histo_indices[-1]]
	...
	y,x = np.mgrid[min_wl-0.05:max_wl-0.05+delta_wl:delta_wl, min_wl-0.05:max_wl-0.05+delta_wl:delta_wl]


	y_b,x_b = np.mgrid[min_wl-0.05:max_wl+delta_wl-0.05:delta_wl, min_wl-0.05:max_wl-0.05+delta_wl:delta_wl]

	#vmax_Alice = np.max(np.array(ch0_bin)[:61*81])
	#vmax_Bob = np.max(np.array(ch1_bin)[:61*81])

	#print(vmax_Alice)
	#print(vmax_Bob)

	Alice_JSI_raw = np.array(ch0_bin[0:81*81]).reshape(81,81)
	Bob_JSI_raw = np.array(ch1_bin[0:81*81]).reshape(81,81)

	#print(Alice_JSI_raw)
	#Alice_JSI = [[] for _ in range(51)]
	#Bob_JSI = [[] for _ in range(51)]

	#for i in range(51):
	#	for j in range(51):
	#		Alice_JSI[i].append(Alice_JSI_raw[i*51+j])
	#		Bob_JSI[i].append(Bob_JSI_raw[i*51+j])
	...
	c = ax.pcolormesh(x,y,Alice_JSI_raw,vmax=11000)
	ax.set_ylim(top=1537.4)
	ax.set_xlim(left=1534.5)
	ax.set_ylabel('Alice 1')
	ax.set_xlabel('Alice 2')
	ax.set_title("Alice")
	...
	c = ax.pcolormesh(x_b,y_b,Bob_JSI_raw,vmax=7000)
	ax.set_ylim(top=1537.4)
	ax.set_xlim(left=1534.5)
	ax.set_ylabel('Bob 2')
	ax.set_xlabel('Bob 1')
