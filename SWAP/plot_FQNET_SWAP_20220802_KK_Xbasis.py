...
START_TIME = '2022-08-02 16:42:14'
END_TIME = '2022-08-03 03:42:14'
table_start_time = "02_08_2022_10_43_30"
tab1_table_name = "TAB1"+table_start_time
tab2_table_name = "TAB2"+table_start_time
print(tab1_table_name)
print(tab2_table_name)

phase_table_name = "kiave_phase_20220802"
...
time_tab1,time_tab2,time_phase=[],[],[]
...
try:
	...
	with db_gui.cursor(pymysql.cursors.DictCursor) as cur:
		...
		TABLE_NAME = phase_table_name
		queryGUI = "SELECT intf1, intf2, datetime FROM "+TABLE_NAME+" WHERE datetime BETWEEN {ts %s} AND {ts %s}"
		cur.execute(queryGUI, (START_TIME,END_TIME,))
		row = cur.fetchone()

		while row is not None:
			intf1.append(row["intf1"])
			intf2.append(row["intf2"])
			time_phase.append(row["datetime"])
			row = cur.fetchone()
	...
	time_phase_first = str(time_phase[0])
	print("time_phase_first=",time_phase_first )
	time_phase_last = str(time_phase[-1])

	first_time_phase = datetime.datetime.strptime(time_phase_first,'%Y-%m-%d %H:%M:%S')
	time_phase_dt = []
	time_phase_el_mins = []
	time_phase_el = []
	...
	for t in time_phase:
		t=str(t)
		datime=datetime.datetime.strptime(t,'%Y-%m-%d %H:%M:%S')
		elapsed = datime - first_time_phase
		time_phase_dt.append(datime)#.time)
		time_phase_el.append(elapsed.total_seconds())
		time_phase_el_mins.append((elapsed.total_seconds())/60) #Convert elapsed time from seconds to minutes
	...
	time_phase_el=np.array(time_phase_el)
	...
	for i in range(len(time_tab2_el)):
		...
		if  i%round(5*60/adqtime_tab2)==0:
			...
			if j%2==1:
				...
				x_noise0_0.append(ch11)
				x_noise0_1.append(ch12)
				x_signal1_0.append(ch13)
				x_signal1_1.append(ch14)
			else:
				x_basis_signal.append(bin)
				x_basis_noise_1.append(bin_1)
				x_signal0_0.append(ch11)
				x_signal0_1.append(ch12)
				x_noise1_0.append(ch13)
				x_noise1_1.append(ch14)
			ch11 = Ch11[i]
			ch12 = Ch12[i]
			ch13 = Ch13[i]
			ch14 = Ch14[i]
	...
	plt.plot(time_phase_el,intf1,label='intf1')
	plt.plot(time_phase_el,intf2,label='intf2')
	plt.ylabel("Phase")
	...
	print("x signal counts: ",x_basis_signal,sum(x_signal0_0),sum(x_signal0_1))
	print("x noise counts: ",x_basis_noise,sum(x_noise0_0),sum(x_noise0_1))

	print("x signal counts 1: ",x_basis_signal_1,sum(x_signal1_0),sum(x_signal1_1))
	print("x noise counts 1: ",x_basis_noise_1,sum(x_noise1_0),sum(x_noise1_1))
