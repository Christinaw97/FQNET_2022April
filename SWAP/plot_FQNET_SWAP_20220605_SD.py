...
START_TIME = '2022-06-05 15:30:00'
END_TIME = '2022-06-05 15:40:00'
table_start_time = "05_06_2022_15_13_23 "
...
try:
	...
	fig2, axs2 = plt.subplots(3,1, num=238, sharex = True)
	tab2_indices= range(len(time_tab2_el))
	tab2_data = [Ch13, Ch14, Ch6]#, [Rb1, Rb2, Rb3], [Rc1, Rc2, Rc3]]
	tab2_titles = ["total el+le & BSM", "total ee+ll & BSM", "BSM"]#, ["Rb1", "Rb2", "Rb3"], ["Rc1","Rc2", "Rc3"]]

	for i in range(len(tab2_titles)):
		axs2[i].plot(time_tab2_el[tab2_indices[0]:tab2_indices[-1]], tab2_data[i][tab2_indices[0]:tab2_indices[-1]],  linestyle = 'none', marker= '.', markersize=8)
		axs2[i].set_ylabel(tab2_titles[i])
	axs2[i].set_xlabel('Elapsed time (s)', fontsize =16)
	...
	tab3_indices= range(len(time_tab2_el))
	tab3_data = [Ch11, Ch12]#, [Rb1, Rb2, Rb3], [Rc1, Rc2, Rc3]]
	tab3_titles = ["total el+le", "total ee+ll"]#, ["Rb1", "Rb2", "Rb3"], ["Rc1","Rc2", "Rc3"]]

	for i in range(len(tab3_titles)):
		axs3[i].plot(time_tab2_el[tab3_indices[0]:tab3_indices[-1]], tab3_data[i][tab3_indices[0]:tab3_indices[-1]],  linestyle = 'none', marker= '.', markersize=8)
		axs3[i].set_ylabel(tab3_titles[i])
	axs3[i].set_xlabel('Elapsed time (s)', fontsize =16)

	##Saving Counts Plots into Disk
	figname = 'zbasis_{}_ee_ll'.format(table_start_time)
	fig3.savefig(figname+".png")
	fig3.savefig(figname+".pdf")

	plt.show()


	fig4, axs4 = plt.subplots(4,1, num=400, sharex = True)
	tab4_indices= range(len(time_tab2_el))
	tab4_data = [Ch0, Ch1, Ch2, Ch3]#, [Rb1, Rb2, Rb3], [Rc1, Rc2, Rc3]]
	tab4_titles = ["Alice Coincidences", "Alice Accidentals", "Bob Coincidences", "Bob Accidentals"]#, ["Rb1", "Rb2", "Rb3"], ["Rc1","Rc2", "Rc3"]]

	for i in range(len(tab4_titles)):
		axs4[i].plot(time_tab2_el[tab4_indices[0]:tab4_indices[-1]], tab4_data[i][tab4_indices[0]:tab4_indices[-1]],  linestyle = 'none', marker= '.', markersize=8)
		axs4[i].set_ylabel(tab4_titles[i])
	axs4[i].set_xlabel('Elapsed time (s)', fontsize =16)

	##Saving Counts Plots into Disk
	figname = 'coinc'.format(table_start_time)
	fig4.savefig(figname+".png")
	fig4.savefig(figname+".pdf")
	...
	print("X-basis Swapping counts: ",x_basis_signal)
